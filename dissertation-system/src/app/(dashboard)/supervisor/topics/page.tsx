...
export default function SupervisorTopicsPage() {
  ...
  async function handleReview(id: string, status: 'APPROVED' | 'REJECTED', reviewNote: string) {
    ...
    if (res.ok) {
      await fetchTopics()
      showToast('success', status === 'APPROVED' ? 'Topic approved — project registered.' : 'Topic rejected.')
    } else if (d.reauth) {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-900 sm:text-2xl">Project Topic Reviews</h1>
        <p className="mt-1 text-sm text-gray-500">
          Review topic proposals from your assigned students. Approve the best one — this automatically registers the student&apos;s project.
        </p>
      </div>
